...
{
    int u18 = ui->comboBox_u18->currentIndex();     // 0 - Auto Match    1 - Manual Match
    int wwvp = ui->comboBox_wwvp->currentIndex();   // 0 - Not Required    1 - Required
    int importance = ui->comboBox_importance->currentIndex();      // 0 - Auto Match    1 - Manual Match
    ...
    // u18 auto match
    // match mentors with WWVP to mentee who is u18
    if (!u18) {
        ...
        while(1) {
            ...
            for (int k=0;k<mentor.rowCount();k++)  // loop each mentor
            {
                ...
                QString mentee_query = "SELECT * FROM mentee WHERE mentor_uid IS NULL AND u18 = 1 ";
                if (importance) {
                ...
                if (maxscore>=0 && !matched_mentee_uid.isEmpty()){
                    QSqlQuery query(db);
                    query.exec(QString("UPDATE mentee SET mentor_uid='%1' WHERE uid='%2'").arg(mentoruid, matched_mentee_uid));
                    end = false;
                }
            }
    ...
    while(1) {
        ...
        if (wwvp) {     // required
            ...
            mentor.setQuery("SELECT * FROM mentor WHERE train_complete = 1;", db);
        }
        while(mentor.canFetchMore()){
            mentor.fetchMore();
        }
        ...
        for (int k=0;k<mentor.rowCount();k++)  // loop each mentor
        {
            ...
            // fetch mentees
            QSqlQueryModel mentee(this);
            QString mentee_query = "SELECT * FROM mentee WHERE mentor_uid IS NULL AND u18 = 0 ";
            if (importance) {
                mentee_query += "AND importance = 0 ";
            }
            mentee.setQuery(mentee_query, db);
